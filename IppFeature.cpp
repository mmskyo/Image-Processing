	...
	for (j = 1; j < h - 1; j++)
		for (i = 1; i < w - 1; i++)
		{
			...
			for (m = 0; m < 3; m++)
				for (n = 0; n < 3; n++)
				{
					sum_y += (pSrc[j - 1 + m][i - 1 + n] * mask_y[m][n]);
				}

			for (m = 0; m < 3; m++)
				for (n = 0; n < 3; n++)
				{
					sum_x += (pSrc[j - 1 + m][i - 1 + n] * mask_x[m][n]);
				}
			val = sqrt(static_cast<double>(sum_x * sum_x + sum_y * sum_y));

			pDst[j][i] = static_cast<BYTE>(limit(val + 0.5));
			/*if (val > 150) {
				pDst[j][i] = 255;
			}
			else {
...
void IppEdgeCanny(IppByteImage& imgSrc, IppByteImage& imgEdge, float sigma, float th_low, float th_high)
{
	...
	for (j = 1; j < h - 1; j++)
		for (i = 1; i < w - 1; i++)
		{
			pGx[j][i] = -pGauss[j - 1][i - 1] - 2 * pGauss[j][i - 1] - pGauss[j + 1][i - 1] -pGauss[j - 1][i] - 2 * pGauss[j][i] - pGauss[j + 1][i]
