...
void IppFIilterLaplacian(IppByteImage& imgSrc, IppByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void IppFilterMin(IppByteImage& imgSrc, IppByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void IppFilterMax(IppByteImage& imgSrc, IppByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
...
void IppFilterWeightedMedian(IppByteImage& imgSrc, IppByteImage& imgDst)
{
	int w = imgSrc.GetWidth();
	int h = imgSrc.GetHeight();
	...
	for (j = 1; j < h - 1; j++) {
		for (i = 1; i < w - 1; i++) {
			m[0] = pSrc[j - 1][i - 1];
			m[1] = pSrc[j - 1][i]; m[2] = pSrc[j - 1][i];
			m[3] = pSrc[j - 1][i + 1];
			m[4] = pSrc[j][i - 1]; m[5] = pSrc[j][i - 1];
			m[6] = pSrc[j][i]; m[7] = pSrc[j][i]; m[8] = pSrc[j][i];
			m[9] = pSrc[j][i + 1]; m[10] = pSrc[j][i + 1];
			m[11] = pSrc[j + 1][i - 1];
			m[12] = pSrc[j + 1][i]; m[13] = pSrc[j + 1][i];
			m[14] = pSrc[j + 1][i + 1];

			std::sort(m, m + 15); //주소를 입력으로 받아 오름차순으로 정렬
			pDst[j][i] = m[7];
		}
	}
}
